...
export default function ProjectRegistrationForm() {
  ...
  const handleImageUpload = async (file: File, type: "before" | "after") => {
    try {
      ...
      if (type === "before") {
        form.setValue("satelliteImagery.before", result.url);
      } else {
        form.setValue("satelliteImagery.after", result.url);
      }
      ...
      toast({
        title: "Upload Failed",
        description: `Failed to upload ${type === "before" ? "before" : "after"} image. Please try again.`,
        variant: "destructive",
      });
    }
  };
